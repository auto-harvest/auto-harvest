...
class CollectorService {
  ...
  private async checkMongoDBVersion() {
    try {
      ...
      this.useAdvancedAggregation = version[0] > 5 || (version[0] === 5 && version[1] >= 2);
      console.log(`MongoDB ${info.version} detected. Advanced aggregation: ${this.useAdvancedAggregation}`);
    } catch (error) {
      console.warn('Could not detect MongoDB version, using fallback aggregation');
  ...
  private async updateSquashedLog(
    ...
    if (existing) {
      ...
      const newAverage = existing.average + (value - existing.average) / newDatapoints;

      // Update variance using Welford's online algorithm
      const delta = value - existing.average;
      const delta2 = value - newAverage;
      const m2 = existing.variance * existing.variance * existing.datapoints + delta * delta2;
      const newVariance = Math.sqrt(m2 / newDatapoints);

      // Update min/max
      const newMin = value < existing.min.value
        ? { value, date: timestamp }
        : existing.min;
      const newMax = value > existing.max.value
        ? { value, date: timestamp }
        : existing.max;
      ...
      const newMedian = (existing.median * existing.datapoints + value) / newDatapoints;
      ...
      if (error.code !== 11000) {
        console.error(`Error updating squashed log for ${interval}:`, error.message);
        throw error;
      }
   ...
   */
  private getIntervalTimestamp(timestamp: Date, interval: 'minute' | 'hour' | 'day' | 'week'): Date {
  ...
  private async groupLogsByInterval(interval: string, dateFormat: string) {
    ...
    console.log(`⚠️  Found ${missedIntervals.length} missed ${interval} intervals, processing...`);
    console.log(`   Missing intervals:`, missedIntervals.map(m => `${m.controller}/${m.type}@${m.interval}`).join(', '));

    // Process only the missed intervals
    if (this.useAdvancedAggregation) {
      return this.processMissedIntervalsAdvanced(missedIntervals, interval, dateFormat);
    }
    return this.processMissedIntervalsFallback(missedIntervals, interval, dateFormat);
  }
  ...
  private async processMissedIntervalsAdvanced(
    missedIntervals: Array<{ type: string; controller: string; interval: string }>,
    interval: string,
    dateFormat: string
  ) {
    ...
    const logs = await sensorLogModel.aggregate([
      ...
      },
      {
        $group: {
          ...
          median: { $median: { input: '$value', method: 'approximate' } } as any,
    ...
    console.log(`✅ Processed ${logs.length} missed ${interval} intervals successfully`);
  }
  ...
  private async processMissedIntervalsFallback(
    ...
    const bulkOps = logs.map((log: {
      ...
      const minEntry = log.sortedValues.find(
        (v) => v.value === log.minValue
      );
      const maxEntry = log.sortedValues.find(
        (v) => v.value === log.maxValue
      );
